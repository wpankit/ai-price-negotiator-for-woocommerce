...
<div class="aipn-order-meta">
		...
		<tr>
			...
			echo esc_html(
				implode(
					' → ',
					array_map(
						function ( $o ) use ( $aipn_currency ) {
							return $aipn_currency . number_format( $o, 2 );
						},
						$data['offers']
					)
				)
	...
	<div class="aipn-breakdown">
		...
		<table>
			...
			<tbody>
				...
				<tr>
					<td><?php echo esc_html( $aipn_item['name'] ); ?>
					<?php
					if ( $aipn_item['quantity'] > 1 ) {
						echo '&times;' . esc_html( $aipn_item['quantity'] );}
					?>
					</td>
	...
	<div id="aipn-chat-log" class="aipn-chat-log">
		<?php
		foreach ( $data['conversation'] as $aipn_msg ) :
